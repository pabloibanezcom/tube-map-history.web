        ...
        {groupBy(connections, 'year', 'connections', true).map(yc => {
          return yc.connections.map((c, cIdx) => {
            return (
              <li
                key={cIdx}
                className={`ms-timeline-item ${
                  cIdx === yc.connections.length - 1 ? 'last-item' : ''
                }`}
                onClick={() => onStationSelected(c.stations[0])}
              >
                <div className="ms-timeline-date">
                  <time className="timeline-time">{!cIdx ? yc.year : null}</time>
                  <i
                    className="ms-timeline-point"
                    style={{ top: `${5}px`, backgroundColor: c.line.colour }}
                  />
                </div>
                <div className="ms-timeline-station-name">{c.stations[0].name}</div>
              </li>
            );
          });
        })}
