          ...
          {user && (
            <div className="row">
              <div className="col-lg-3">
                ...
                <Button color="secondary" text="Edit user" block outline />
              </div>
              <div className="col-lg-9">
                {user && user.drafts && (
                  <div className="mb-20">
                    <h3 className="mb-10">
                      Your published drafts ({user.drafts.filter(d => d.isPublished).length})
                    </h3>
                    <Slider items={3}>
                      {user.drafts
                      ...
                      {!user.drafts.length && (
                        <NoResultsBox className="ml-15" noDrafts={!user.drafts.legnth} />
                      )}
                    </Slider>
                  </div>
                )}
                {user && user.drafts && (
                  <div className="mb-20">
                    <div className="flex flex-space-between">
                      <h3 className="mb-10">
                        Your other drafts ({user.drafts.filter(d => !d.isPublished).length})
                      </h3>
                      <Button
                        type="link"
                ...
                {towns && (
                  <div className="mb-20">
                    ...
                    <Slider items={4}>
                      {towns.map(town => (
                        <div className="pr-15" key={town._id}>
                          <a href={`/admin/town/${town.url}`}>
                            <TownCard town={town} />
                          </a>
                        </div>
                      ))}
