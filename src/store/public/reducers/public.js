...
export const publicReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    // Main
    case mainActions.INIT_MAP_START:
      return defaultStart(state, action);
    case mainActions.INIT_MAP_SUCCESS:
      return mainReducers.initMapSuccess(state, action);
    case mainActions.INIT_MAP_FAIL:
      return defaultFail(state, action);

    case mainActions.INIT_MAP_DRAFT_START:
      return defaultStart(state, action);
    case mainActions.INIT_MAP_DRAFT_SUCCESS:
      return mainReducers.initMapSuccess(state, action);
    case mainActions.INIT_MAP_DRAFT_FAIL:
      return defaultFail(state, action);

    case mainActions.SET_YEAR:
      return mainReducers.setYear(state, action);
